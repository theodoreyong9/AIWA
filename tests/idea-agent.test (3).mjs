...
// ── Mechanism patterns: real, code-derived, never a code template ──

test('collectContextSnapshot passes mechanismPatterns through untouched, defaulting to null', () => {
  const registry = registryWith([]);
  const patterns = { totalModulesMined: 3, commonCtxPatterns: [{ call: 'postCausalEvent', freq: 0.66, count: 2 }], unusedCtxPrimitives: ['transferClaim'] };
  const withPatterns = collectContextSnapshot(registry, 'alice', [], {}, null, patterns);
  assert.deepEqual(withPatterns.mechanismPatterns, patterns);

  const withoutPatterns = collectContextSnapshot(registry, 'alice', []);
  assert.equal(withoutPatterns.mechanismPatterns, null);
});

test('buildIdeaSystemPrompt honestly states nothing has been mined yet rather than fabricating a pattern', () => {
  const snapshot = collectContextSnapshot(registryWith([]), 'alice', []); // no mechanismPatterns passed
  const prompt = buildIdeaSystemPrompt(snapshot);
  assert.match(prompt, /MECHANISM PATTERNS: none mined yet/);
});

test('buildIdeaSystemPrompt includes real common and never-used ctx primitives when mining data is present', () => {
  const patterns = { totalModulesMined: 4, commonCtxPatterns: [{ call: 'postCausalEvent', freq: 0.75, count: 3 }], unusedCtxPrimitives: ['sendToPeer', 'onPeerMessage'] };
  const snapshot = collectContextSnapshot(registryWith([]), 'alice', [], {}, null, patterns);
  const prompt = buildIdeaSystemPrompt(snapshot);
  assert.match(prompt, /ctx\.postCausalEvent \(75%\)/);
  assert.match(prompt, /ctx\.sendToPeer, ctx\.onPeerMessage/);
  assert.match(prompt, /4 already-registered, hash-verified real module/);
});

test('buildIdeaSystemPrompt never presents mechanism-pattern data as a code template — only frequency text', () => {
  const patterns = { totalModulesMined: 2, commonCtxPatterns: [{ call: 'toast', freq: 1, count: 2 }], unusedCtxPrimitives: [] };
  const snapshot = collectContextSnapshot(registryWith([]), 'alice', [], {}, null, patterns);
  const prompt = buildIdeaSystemPrompt(snapshot);
  assert.ok(!prompt.includes('(function'), 'the prompt must never contain a code skeleton — only descriptive frequency text');
  assert.ok(!prompt.includes('skeleton'));
});

test('sanitizeIdeaReply also catches leakage of the MECHANISM PATTERNS instruction phrase', () => {
  const leaked = 'A great module idea.\n\nMECHANISM PATTERNS suggest this is underused.';
  const cleaned = sanitizeIdeaReply(leaked);
  assert.equal(cleaned, 'A great module idea.');
});
