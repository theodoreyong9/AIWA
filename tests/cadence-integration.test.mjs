...
test('cadence reducer over a real EventDag: valid chain advances, a skipped epoch is rejected', async () => {
  ...
  await dag.addEvent([c2], { type: 'cadence', domain: 'mars-colony-1', epoch: 4 }); // invalid: skips 3
...
test('cadence reducer is stable across two DAGs merged in either order', async () => {
  ...
  const a1 = await dagA.addEvent([genesis], { type: 'cadence', domain: 'domain-a', epoch: 1 });
  await dagA.addEvent([a1], { type: 'cadence', domain: 'domain-a', epoch: 2 });

  const dagB = new EventDag();
  await dagB.addEvent([], { type: 'genesis' }); // same payload -> same id as dagA's genesis
  const b1 = await dagB.addEvent([genesis], { type: 'cadence', domain: 'domain-b', epoch: 1 });
  await dagB.addEvent([b1], { type: 'cadence', domain: 'domain-b', epoch: 2 });
