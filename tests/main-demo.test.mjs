...
const theta = { reward: { alpha: 1, beta: 0, gamma: 1, C: Math.E - 1, minQ: 1 }, budgets: { earth: 1000, mars: 1000 } };
...
class DomainReplica {
  constructor(name, dag) {
    ...
    this.epoch = 0;
    this.pending = [];
    this.lastVdfOutput = undefined; // real cadence VDF chaining (R11) — see cadence-vdf-helper.mjs
  }
  async advanceCadence() {
    const nextEpoch = this.epoch + 1;
    const parents = [...new Set([this.lastCadenceId ?? this.genesisId, this.lastEventId])];
    const payload = cadencePayload(this.name, nextEpoch, this.lastVdfOutput);
    const id = await this.dag.addEvent(parents, payload);
    this.lastCadenceId = id;
    this.lastEventId = id;
    this.epoch = nextEpoch;
    this.lastVdfOutput = payload.vdfOutput;
    return id;
  }
  commit(amount) {
