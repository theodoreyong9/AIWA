...
test('SECURITY/PARITY: setAuditStatus rejects an invalid status string instead of accepting anything', () => {
  let { state } = registerModule(initialModuleRegistryState(), baseEntry());
  const result = setAuditStatus(state, 'mymodule.js', 'not-a-real-status');
  assert.equal(result.accepted, false);
  assert.match(result.reason, /not a valid audit status/);
  assert.equal(result.state.modules['mymodule.js'].auditStatus, 'unaudited', 'the invalid write must not have applied');
});

test('setAuditStatus still accepts every real status value (unaudited, passed, red-listed) — matches Rust\'s own AuditStatus enum exactly', () => {
  let { state } = registerModule(initialModuleRegistryState(), baseEntry());
  for (const status of ['unaudited', 'passed', 'red-listed']) {
    const result = setAuditStatus(state, 'mymodule.js', status);
    assert.equal(result.accepted, true, `'${status}' must be accepted`);
    state = result.state;
    assert.equal(state.modules['mymodule.js'].auditStatus, status);
  }
});
