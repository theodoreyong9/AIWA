...
test('accrual after cadence has advanced accrues reward proportional to elapsed epochs', () => {
  ...
  const c1 = cadenceEvent('c1', [], 'd1', 1);
  state = applyGEvent(theta, state, c1);
  state = applyGEvent(theta, state, cadenceEvent('c2', ['c1'], 'd1', 2, c1.payload.vdfOutput));
...
test('scarcity clamp applies to the composed reward, not just raw amount requested', () => {
  ...
  const c1 = cadenceEvent('c1', [], 'd1', 1);
  state = applyGEvent(tightTheta, state, c1);
  state = applyGEvent(tightTheta, state, cadenceEvent('c2', ['c1'], 'd1', 2, c1.payload.vdfOutput));
...
test('processing order (cadence before accrual) matters causally, matching real topo order', () => {
  ...
  const events = [
    ...
    c1,
    cadenceEvent('c2', ['c1'], 'd1', 2, c1.payload.vdfOutput),
  ];
