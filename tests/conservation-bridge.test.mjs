...
async function buildDomainWith50(dag, domainId) {
  ...
  let last = genesis, lastCadence = null, previousVdfOutput;
  for (let e = 1; e <= 5; e++) {
    const parents = [...new Set([lastCadence ?? genesis, last])];
    const payload = cadencePayload(domainId, e, previousVdfOutput);
    const id = await dag.addEvent(parents, payload);
    lastCadence = id; last = id; previousVdfOutput = payload.vdfOutput;
  }
...
test('a signed transfer moves real ownership from alice to bob, verified for real', async () => {
  ...
  const transferEvent = await buildSignedTransferEvent({ claimId: 'c1', from: aliceId, to: bobId }, alice.seed, alice.pubkeyBytes);
...
test('SECURITY: a forged transfer (attacker claims a from they do not control) is rejected — the vulnerability this file was built to close', async () => {
  ...
  const forgedEvent = await buildSignedTransferEvent({ claimId: 'c1', from: aliceId, to: 'attacker-domain' }, attacker.seed, attacker.pubkeyBytes);
...
test('SECURITY: a real signature over tampered fields (wrong claimId) is rejected', async () => {
  ...
  const legitEvent = await buildSignedTransferEvent({ claimId: 'c1', from: aliceId, to: 'bob' }, alice.seed, alice.pubkeyBytes);
...
test('a replayed transfer nonce is rejected — cannot re-apply the same signed transfer twice', async () => {
  ...
  const event1 = await buildSignedTransferEvent({ claimId: 'c1', from: aliceId, to: 'bob' }, alice.seed, alice.pubkeyBytes, { nonce: 'fixed-nonce' });
...
test('a real, sufficiently-funded claim-issue is unaffected by the gRejectedEventIds cross-check', async () => {
  ...
  let previousVdfOutput;
  for (let e = 1; e <= 5; e++) {
    const parents = [...new Set([lastCadence ?? last, last])];
    const payload = cadencePayload('alice', e, previousVdfOutput);
    const id = await dag.addEvent(parents, payload);
    lastCadence = id; last = id; previousVdfOutput = payload.vdfOutput;
  }
