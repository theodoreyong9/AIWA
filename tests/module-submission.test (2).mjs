...
test('a first-ever submission from an author is always eligible, and records their rank', async () => {
  ...
  const code = 'const x = 1;';
  const codeHash = await computeModuleHash(code);
  const event = buildSubmissionEvent(baseFields({ codeHash }), signer.seed, signer.pubkeyBytes);
...
test('a second NEW module id from the same author with a declining ratio is rejected', async () => {
  ...
  const event1 = buildSubmissionEvent(baseFields({ moduleId: 'first.js', codeHash: hash1 }), signer.seed, signer.pubkeyBytes);
  ...
  const event2 = buildSubmissionEvent(baseFields({ moduleId: 'second.js', codeHash: hash2 }), signer.seed, signer.pubkeyBytes);
...
test('eligibility does NOT gate an update to a module the author already owns', async () => {
  ...
  const hashV1 = await computeModuleHash(codeV1);
  const eventV1 = buildSubmissionEvent(baseFields({ codeHash: hashV1 }), signer.seed, signer.pubkeyBytes);
  ...
  const hashV2 = await computeModuleHash(codeV2);
  const eventV2 = buildSubmissionEvent(baseFields({ codeHash: hashV2, nonce: crypto.randomUUID() }), signer.seed, signer.pubkeyBytes);
...
test('recordNonce preserves lastSubmissionByAuthor for OTHER authors — a real bug found via end-to-end testing, not by inspection', async () => {
  ...
  const aliceEvent = buildSubmissionEvent(baseFields({ moduleId: 'alice-mod.js', codeHash: aliceHash }), alice.seed, alice.pubkeyBytes);
  ...
  const bobEvent = buildSubmissionEvent(baseFields({ moduleId: 'bob-mod.js', codeHash: bobHash }), bob.seed, bob.pubkeyBytes);
...
test('SECURITY: a different signer cannot "update" (hijack) a module they did not register — the exact vulnerability found and closed', async () => {
  ...
  const aliceHash = await computeModuleHash(aliceCode);
  const aliceEvent = buildSubmissionEvent(baseFields({ moduleId: 'weather.js', codeHash: aliceHash }), alice.seed, alice.pubkeyBytes);
  ...
  const evilEvent = buildSubmissionEvent(baseFields({ moduleId: 'weather.js', codeHash: evilHash }), attacker.seed, attacker.pubkeyBytes);
...
test('the original author CAN still update their own module — the fix gates hijacking, not legitimate updates', async () => {
  ...
  const codeV1 = 'const v = 1;';
  const hashV1 = await computeModuleHash(codeV1);
  const eventV1 = buildSubmissionEvent(baseFields({ moduleId: 'weather.js', codeHash: hashV1 }), alice.seed, alice.pubkeyBytes);
  ...
  const codeV2 = 'const v = 2;';
  const hashV2 = await computeModuleHash(codeV2);
  const eventV2 = buildSubmissionEvent(baseFields({ moduleId: 'weather.js', codeHash: hashV2, nonce: crypto.randomUUID() }), alice.seed, alice.pubkeyBytes);
...
test('a NEW module id remains open to any author — the hijack fix applies only to already-registered ids, matching §27.4\'s open-registration design', async () => {
  ...
  const aliceCode = 'const a = 1;';
  const aliceHash = await computeModuleHash(aliceCode);
  const aliceEvent = buildSubmissionEvent(baseFields({ moduleId: 'alice-only.js', codeHash: aliceHash }), alice.seed, alice.pubkeyBytes);
  const r1 = await submitModule(initialModuleRegistryState(), initialSubmissionState(), aliceEvent, aliceCode, {
    registerModuleFn: registerModule, updateModuleCodeFn: updateModuleCode,
  });
  ...
  const bobHash = await computeModuleHash(bobCode);
  const bobEvent = buildSubmissionEvent(baseFields({ moduleId: 'bob-new.js', codeHash: bobHash }), bob.seed, bob.pubkeyBytes);
