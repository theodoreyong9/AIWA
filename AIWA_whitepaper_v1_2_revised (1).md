...
Open work this revision creates, stated plainly rather than left implicit: §24.1's Sybil-profit analysis and Appendix C's Lemma 1 divergence experiments were both derived against the prior power-law reference function, specifically exploiting its b^α term. In the structure above, b appears linearly (exponent 1) — α now governs q, not b — so §24.1's R_N = K·B^α·N^(1−α) derivation does not carry over as written, and neither do Appendix C's measured divergence percentages, which were empirical results for that specific formula. An informal observation, not a proven result: linear scaling in b makes reward exactly neutral to splitting a fixed capital B across N identities before identity cost is considered (N·(B/N)·q^α = B·q^α, independent of N), which would make any c_id > 0 strictly splitting-discouraging rather than only above a computed N* — but this has not been rigorously re-derived, and should be treated as a conjecture prompted by inspection, not a verified claim, until it is. Recorded as open work, in the same spirit as §9.1 and §29.10's findings: a formula change with this much algebraic consequence needs to be re-verified, not assumed to inherit the prior formula's proofs. Executable detail and cross-language parity for the new formula itself (not the still-open Sybil re-derivation) are in Appendix H.14.
...
24.1 Adding identity cost to the fixed-capital case. [Note on this section as of the current revision: this derivation, and §24.2–24.3's that build on it, are stated for the power-law reference form r(b,q)=K·b^α·q^β used prior to §10's Proof-of-Will revision, where α governs b's exponent directly. §10's current reference formula applies α to q instead, with b entering linearly — the algebra below has not been re-derived against that structure, and should not be assumed to transfer; see §10's own note on this. Retained here as the derivation for deployments that choose the simpler power-law form, which §10 still permits as a valid configuration, not only as historical content.] Let an attacker hold total capital B , split evenly across N identities ( b = B/N each), and pay a fixed per-identity creation cost c_id > 0 . Profit as a function of N , for fixed B and elapsed age t :
...
What remains open, stated precisely: checkSubmissionEligibility() is not
yet called anywhere inside module-submission.js's validateSubmission()
— it exists, tested, but unwired, a specific and named gap rather than
an implied one. Likewise, no application UI yet displays the module list
sorted by this rank, or exposes the burn-registration flow required to
have a rank at all outside of test fixtures.
