...
mod tests {
    ...
    fn submit_module_end_to_end_registers_new_module() {
        ...
        let outcome = submit_module(&mut registry, &mut submissions, &event, code, 1000);
    ...
    fn submit_module_second_submission_updates_and_resets_audit() {
        ...
        let mut registry = ModuleRegistryState::new();
        let mut submissions = SubmissionState::default();
        submit_module(&mut registry, &mut submissions, &event1, code_v1, 1000);
        ...
        let outcome2 = submit_module(&mut registry, &mut submissions, &event2, code_v2, 1001);
