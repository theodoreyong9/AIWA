...
One assumption recorded rather than silently relied upon: unlike
id-parity.json for ledger events (§8.1, Appendix H.5), there is not yet
an automated cross-language parity script proving a submission signed
...
functions were written to produce byte-identical output by careful,
manual field-order matching; this has not been mechanically checked
against a real cross-language signature exchange. Recorded as open work,
in the same spirit as §9.1/§29.10's materialization-order finding: a
claim of interoperability needs to be demonstrated, not merely intended.
