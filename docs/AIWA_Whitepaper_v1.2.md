...
- [3. Contributions, in Detail](#3-contributions-in-detail)
- [4. Related Work](#4-related-work)
- [5. System Model](#5-system-model)
- [6. Six Different Problems Routinely Conflated Into One](#6-six-different-problems-routinely-conflated-into-one)
- [7. Conservation: Proof-Carrying Transfer](#7-conservation-proof-carrying-transfer)
- [8. Autonomous Accrual as a Replicated Data Type](#8-autonomous-accrual-as-a-replicated-data-type)
- [9. Replicated State Is Not the Economic View](#9-replicated-state-is-not-the-economic-view)
- [9.3 Determinism Is Not Consensus](#93-determinism-is-not-consensus)
- [18.1 Adversarial Test Matrix](#181-adversarial-test-matrix)
- [Proof of Cost, Proof of Control, and Real-World Identity](#proof-of-cost-proof-of-control-and-real-world-identity)
- [27.x Module Security Boundary and Adversarial Coverage](#27x-module-security-boundary-and-adversarial-coverage)
- [Security Status at a Glance](#security-status-at-a-glance)
- [10. The Accrual Function, With Cadence-Derived Economic Time](#10-the-accrual-function-with-cadence-derived-economic-time)
- [11. Identity and the Weak-Identity Lemma](#11-identity-and-the-weak-identity-lemma)
- [12. The Partition Issuance Bound](#12-the-partition-issuance-bound)
- [13. Where the Scarcity Constraint Must Go](#13-where-the-scarcity-constraint-must-go)
- [14. Reconciliation](#14-reconciliation)
- [15. Historical Commitments](#15-historical-commitments)
- [16. Observability and the Evidence–Autonomy Trade-off](#16-observability-and-the-evidenceautonomy-trade-off)
- [17. Claim–Evidence–Assumption Matrix](#17-claimevidenceassumption-matrix)
- [18. Threat Model](#18-threat-model)
- [19. Experimental Method](#19-experimental-method)
- [20. Experiment 1 — Post-Partition Convergence](#20-experiment-1--post-partition-convergence)
- [21. Experiment 2 — Deliberately Broken Invariants](#21-experiment-2--deliberately-broken-invariants)
- [22. Weak Identity](#22-weak-identity-confirming-lemma-1-and-a-methodological-caution)
- [23. What the Experiments Establish](#23-what-the-experiments-establish)
- [24. Economic Security](#24-economic-security-a-cost-explicit-sybil-and-patient-capital-analysis)
- [25. Transport](#25-transport-pluggable-partition-tolerant-by-requirement)
- [26. Identity](#26-identity)
- [27. Modules](#27-modules-sandboxed-extensibility-without-elevated-trust)
- [28. Advisory AI and Hyperprofile](#28-advisory-ai-and-hyperprofile)
- [29. Open Problems](#29-open-problems)
- [30. Design Principles](#30-design-principles)
- [31. Discussion](#31-discussion)
- [32. What This Architecture Does Not Claim](#32-what-this-architecture-does-not-claim)
- [33. Conclusion](#33-conclusion)
- [Appendix A — Notation](#appendix-a--notation)
- [Appendix B — Formal Specification and Reference Pseudocode](#appendix-b--formal-specification-and-reference-pseudocode)
- [Appendix C — Counterexamples](#appendix-c--counterexamples)
- [Appendix D — Worked Sybil-Cost and Issuance Examples](#appendix-d--worked-sybil-cost-and-issuance-examples)
- [Appendix E — Security Property Registry](#appendix-e--security-property-registry-full)
- [Appendix F — Threat-to-Guarantee Matrix](#appendix-f--threat-to-guarantee-matrix)
- [Appendix G — Reproducibility and Verification Checklist](#appendix-g--reproducibility-and-verification-checklist)
- [Appendix H — Implementation Evidence](#appendix-h--implementation-evidence)

---

## How to read this paper

The paper has three layers of depth:

1. **Architecture** — the diagrams and section summaries explain the system without requiring implementation details.
2. **Protocol** — equations, invariants, state transitions, and threat models define what the system must preserve.
3. **Evidence** — experiments, test vectors, implementation notes, and appendices show what has actually been exercised.

The most important distinction throughout is between **what follows from the model**, **what is implemented**, and **what remains conditional or open**.
...
flowchart TB
    T[Transport
    delay-tolerant / local mesh] --> L[Event DAG H_d]
    I[Identity
    Ed25519 + c_id] --> L
    L --> G[Materialized state
    A = G(H_d, θ)]
    G --> E[Economics
    cadence + reward + scarcity]
    G --> C[Conservation
    claims + transfers]
    G --> M[Module registry
    content-addressed code]
    M --> S[Sandboxed modules
    iframe + ctx]
    G --> P[Public profile]
    P --> AI[Advisory AI]
    AI -. never authoritative .-> G
    S -. untrusted code .-> AI
    T <--> L
```

The architectural boundary is deliberate: **events are replicated; state is materialized; authority remains in deterministic reducers; advisory components never decide validity.**
...
### 3.1 Separation of replicated state from economic meaning.

Every AIWA domain's ledger state is a set of uniquely identified events, H_d ; economic meaning is a deterministic materialized view A_d = G(H_d, θ) computed over that set. A scalar balance is never itself the replicated object — this

single design choice is what makes convergence provable rather than merely hoped for (§9).

### 3.2 A proved impossibility result for autonomous issuance.

For any set of domains whose issuance rates are bounded away from zero, unrestricted autonomy, independent per- domain decisions, full retention of locally valid issuance, and a fixed global supply bound are jointly incompatible under arbitrarily long partition (Proposition 1, §12). The result is stated precisely as what kind of result it is: a short, checkable consequence of a stated set of economic assumptions, not a deep distributed-systems impossibility theorem — its value is in pinning down exactly which assumption has to give, not in its proof difficulty.

### 3.3 An identity-sufficiency lemma usable by any module author.

Lemma 1 (§11) gives an exact, checkable condition for when a coarse (cheap) event identifier is safe for a given reward function, and when it must be replaced by a finer one. This turns "how strong should identity be" from a guess into a per-module calculation.

### 3.4 Executable validation, not assertion.

Sections 18–22 report five experiments run against a working reference implementation of the tested ledger primitives: convergence under adversarial delivery, four deliberately broken invariants (with real counterexamples), and a direct empirical confirmation of Lemma 1 on a concrete reward function. All numbers reported are the actual output of the code in Appendix H, run at the time of writing.

### 3.5 A cost-explicit Sybil and patient-capital model.

§24 shows that reward curvature alone — and, specifically, the mistaken assumption that a diminishing-returns reward function automatically discourages splitting into
...
### 3.6 A reference seven-layer architecture.

§25–28 extend the ledger core into transport, identity, sandboxed third-party modules, and an advisory AI layer, each with the same discipline: what is guaranteed, what is assumed, and what is explicitly out of scope.
...
| Symbol | Meaning |
|---|---|
| `H_d` | Domain event set |
| `A_d` | Materialized economic/application state |
| `C_d` | Latest historical commitment |
| `B_d` | Local issuance constraint |
| `θ_d` | Economic parameters |
| `C(t)` | Communication availability between domains |

AIWA requires local state evolution to remain independent during `C(t)=0`; reconciliation happens only when communication becomes available.
...
These concerns interact, but none is allowed to silently inherit the trust assumptions of another.


### 6.1 Conservation.

A transfer moves ownership of an existing claim, x_A → x_B , without creating a new unit. A transmutation converts x_A → y_B subject to an authorized derivation function f . Neither creates value; both require that the same claim cannot be consumed twice.

### 6.2 Accrual.

S_d(t+Δ) = F(S_d(t), E_d, θ) — new monetary state created from a domain's own local activity, with no existing object consumed. This is structurally different from conservation and requires a different mechanism (§8), not a variant of the same one.

### 6.3 Evidence.

C_n = H(protocol, genesis, d, n, C_{n−1}, A_n) — a commitment that makes retroactive modification of a domain's accepted history detectable. It does not, on its own, establish that the underlying event physically occurred.

### 6.4 Observability.

A mandatory heartbeat turns silence into a protocol-visible event, but cannot by itself determine the cause of silence (blackout, routing failure, node failure, malicious suppression, congestion, clock failure). Silence is not the same thing as any particular cause.

### 6.5 Extensibility.

Third-party code should be able to add features — a game, a dashboard, a trading interface — without that code being trusted with money movement, other users' data, or the transport layer. This is an access-control problem, orthogonal to the four above, and AIWA treats it as its own layer (§27) rather than folding it into "security" generically.

### 6.6 Presentation.

What a user sees and how they interact with it should be swappable independently of everything above — a design constraint that turns out to matter for interplanetary deployments specifically, since a settlement with limited bandwidth or specialized hardware may need a radically different interface to the same underlying state.
...
```mermaid
flowchart TB
    A[Existing claim] --> D[Deactivate]
    D --> P[Prove]
    P --> V[Verify]
    V --> C[Consume]
    C --> X[Activate]
    X --> B[New claim state]
    C -. "count Consume(p) ≤ 1" .-> C
```

The pipeline deliberately separates the movement of an existing claim from the creation of new economic state. Conservation can therefore be checked independently from the accrual function.
...
stateDiagram-v2
    ...
    Deactivated --> Proven: Prove
    Proven --> Verified: Verify
    Verified --> Consumed: Consume
    Consumed --> Active: Activate destination claim
    Verified --> [*]: reject
```

**Load-bearing invariant:** `count(Consume(p)) ≤ 1`.
...
### Event-set replication

```mermaid
flowchart LR
    EA[Domain A\nlocal events] --> U[Union / merge]
    EB[Domain B\nlocal events] --> U
    EC[Domain C\nlocal events] --> U
    U --> H[Converged event set H]
    H --> G[Deterministic materialization G(H, θ)]
```

Replication operates on events. Economic meaning is derived afterwards by deterministic materialization.
...
H_A ∪ H_B]
    B[Domain B
H_B] --> U
    U --> O[Canonical topological order]
    O --> F[Deterministic fold G]
    F --> S[Materialized state A]
```

The replicated object is the event set, not a mutable balance.
...
```mermaid
flowchart TB
    E[Content-addressed events] --> H[H_d\nreplicated set]
    H --> O[Canonical topological order]
    O --> F[Fold / reducers]
    F --> A[A_d = G(H_d, θ)\nmaterialized state]
    A --> UI[Balances, claims, registry, profiles]
```

A derived balance is never the authoritative replicated object. The event set is.

### Materialization pipeline

```mermaid
flowchart TB
    H[Converged event set H_d] --> ENC[Canonical event identity]
    ENC --> ORD[Canonical materialization order]
    ORD --> RED[Reducers]
    TH[Immutable parameters θ] --> RED
    RED --> VIEW[Materialized state A = G(H_d, θ)]
```

Two implementations must agree on both the bytes being hashed and the order in which causally compatible events are folded.
...
None of (i), (iii), (iv), or (v) are gaps introduced by carelessness — each is exactly the kind of decision a protocol specification is supposed to make explicitly, and this paper has so far made each one only implicitly, by what its own two reference implementations happen to agree on. "JS and Rust agree" is real, tested evidence that a workable convention exists; it is not the same claim as "the protocol specifies this," and this paper does not make the second claim on the strength of the first. Recorded as open work, alongside §29.9, as the second most significant open item after §10.1/§24.1's Sybil-resistance gap — see §17's matrix for both stated at the same level of visibility as this paper's stronger results.

### 9.3 Determinism Is Not Consensus

A canonical fold makes the materialized result deterministic **for a given event set**. That does not, by itself, prove consensus on which event set is admissible. The event DAG supplies replicated-state convergence under its delivery assumptions; it does not automatically solve Byzantine admission, permanent message loss, conflicting external facts, incompatible protocol implementations, or the semantic correctness of concurrent branches.

The reference implementations use the same id-sorted depth-first materialization rule and agree on the tested fixtures. That is interoperability evidence, not by itself a normative consensus specification. Canonical event encoding, canonical materialization order, numeric semantics, protocol versioning, and a formal conformance suite therefore remain explicit protocol-level obligations.

```mermaid
flowchart LR
    H[Same converged event set H_d] --> F1[Deterministic fold]
    H --> F2[Another deterministic fold]
    F1 --> A1[Same state if rules match]
    F2 --> A2[Same state if rules match]
    X[Different admissible event sets] --> Y[Determinism alone does not reconcile them]
    Y --> Z[Consensus / admission is a separate property]
```
...
```mermaid
flowchart TB
    S[Previous cadence state] --> V[Sequential VDF]
    V --> CE[Cadence event\nq ← q + 1]
    CE --> Q[Domain cadence q]
    Q --> R[Proof-of-Will reward r(b,q,q_total,T)]
    R --> SC[Scarcity policy]
    SC --> BAL[Materialized balance]
```

Economic time is derived from accepted cadence events, not from a wall clock.

### Economic time

```mermaid
flowchart LR
    PREV[Previous cadence output] --> VDF[Sequential SHA-256 chain]
    VDF --> CAD[Valid cadence event
q := q + 1]
    CAD --> Q[q = q_d - q_0]
    Q --> R[Proof-of-Will reward r(b,q,q_total,T)]
    R --> SC[Scarcity policy]
...
time-sensitive formulation. Under the the wall-clock formulation formulation, a manipulable local
clock could inflate t and therefore reward. In a wall-clock-based formulation, changing the wall clock
...
constructs the wall-clock-dependent variant directly, shows it
...
The reference implementation makes a formula a minted, permanent object: a 'formula-register' event binds an id to a fixed (α, β, γ, C, minQ) forever — no update path exists, by design, unlike a module's code (§27.4), which can be revised while keeping its id. The same id can never be re-minted with different parameters; a second registration attempt for an already-minted id is rejected outright, the same content-addressing discipline §8.1 already establishes for event ids, applied here to an economic object instead. §10's reference formula — the Proof-of-Will structure — is not itself minted through this mechanism: it is 'genesis', a fixed protocol default available to every domain with no event and no cost, specifically to avoid a bootstrapping paradox a strict reading would otherwise create — a domain needs some formula to accrue anything under real economic rules before it could ever afford to mint a new one. Every other formula is a genuine mint, gated by the same identity-cost requirement §24.1's splitting-incentive analysis already motivates for issuing modules, applied here at the application layer rather than inside the pure reducer — mirroring exactly how checkSubmissionEligibility (§27.4.1) is wired in, not a new pattern invented for this. Which minted formula a domain currently uses is a separate, local, non-permanent choice, distinct from whether that formula exists and what its parameters permanently are — switching which formula you use is not the same operation as changing one that exists, and this paper is careful to keep those two questions answered by two different mechanisms rather than conflating them. Executable verification of formula switching is in Appendix H.21.

## 11. Identity and the Weak-Identity Lemma

### Identity strength follows economic semantics

```mermaid
flowchart LR
    P[Event payload] --> ID[Candidate identifier]
    Q[Variables used by reward function] --> ID
    ID --> T{Does identifier preserve
all reward-relevant distinctions?}
    T -- yes --> SAFE[Identifier is sufficient]
    T -- no --> WEAK[Weak identity\ncollision can change reward]
```

The lemma is intentionally semantic: the required identity strength is determined by what the economic function can distinguish.


> **Lemma 1 — Identity sufficiency.**
> A deduplication identity is safe exactly when events it merges are economically indistinguishable to the materialization function. Under
...
The relevant temporal information is the cadence epoch, not an
...
### What cannot coexist indefinitely

```mermaid
flowchart TB
    A[Arbitrarily long partition] --> X{Four properties retained?}
    B[Unrestricted local issuance] --> X
    C[Independent per-domain decisions] --> X
    D[Full retention of locally valid issuance] --> X
    E[Fixed global supply bound] --> X
    X -->|all simultaneously| N[Contradiction]
    X -->|one constraint relaxed| W[Architecture remains possible]
```

The impossibility result is useful because it identifies the constraint that must be named rather than pretending that partition tolerance removes scarcity.

### The impossibility in one picture

```mermaid
flowchart LR
    P[Arbitrarily long partition] --> D1[Domain 1
positive issuance]
    P --> D2[Domain 2
positive issuance]
    D1 --> SUM[Global issuance grows with T]
    D2 --> SUM
    SUM --> X[No fixed finite global bound]
```

To preserve a bound, at least one assumption must change: preallocated budget, convergent issuance, expiry, scarce external resource, or authority.
...
> **Proposition 1 — Finite-supply impossibility.**
> If autonomous issuance remains positive during an arbitrarily long partition while all locally valid issuance is retained, a finite global issuance bound cannot hold. If cumulative issuance rates satisfy ρ_d(t) ≥ ρ_d^min > 0 throughout an arbitrarily long partition, and (C) and (D) both hold, then no finite global supply bound can hold for all partition durations.
...
### Scarcity is a policy boundary

```mermaid
flowchart LR
    B[Local issuance attempt] --> R[Reward function]
    R --> S[Scarcity policy]
    S -->|within budget| I[Issue]
    S -->|budget exhausted| Z[Clamp / reject]
    I --> H[Event DAG]
    Z --> H
```

The scarcity mechanism belongs at the point where issuance becomes state, not as an after-the-fact interpretation of already-created balances.


### 13.1 Preallocated budgets.

B_d(epoch) ; autonomy holds until the local allocation is exhausted.

### 13.2 Rate limits.

ρ_d ≤ ρ_max bounds issuance over any finite interval but not as T → ∞ . RateBound ≠ SupplyBound — confirmed numerically in Appendix H.4, Policy C, and worth stating plainly because it is a common mistake in from-scratch reward-formula design.

### 13.3 Expiring issuance rights.

A right carrying t_exp allows autonomous operation while bounding simultaneously-valid monetary state — at the cost that autonomy no longer equals permanent validity. Confirmed numerically in Appendix H.4, Policy D: I(T) still grows unboundedly, but S(T) converges to a finite steady state, ρ · t_exp , once T exceeds the expiry window.

### 13.4 Scarce physical or computational resources.

Issuance tied to capital, energy, storage, computation, or attested hardware — the constraint is externalized into a scarce resource whose own acquisition cost then matters (§24).

### 13.5 External authority.

Periodic governance-allocated issuance rights — sacrifices communication independence entirely for stronger central control; AIWA supports this as a configuration option but does not recommend it as a default, since it directly contradicts partition autonomy (G_A in §2) for the domain awaiting allocation.
...
```mermaid
flowchart LR
    A[Domain A\nH_A] --> M[Merge by event identity]
    B[Domain B\nH_B] --> M
    M --> H[H_A ∪ H_B]
    H --> G[Deterministic materialization]
    G --> SA[State view A]
    G --> SB[State view B]
```

Reconciliation does not choose one mutable balance over another. It merges evidence and recomputes the view.
...
### Commitment is evidence of history, not physical truth

```mermaid
flowchart TB
    E[Accepted event history] --> K[Canonical serialization]
    K --> H[Cryptographic hash / commitment]
    H --> C[Historical commitment]
    C --> CHECK[Later consistency check]
    CHECK -. cannot prove .-> PHYS[Physical truth of the event]
```

This distinction is fundamental: tamper evidence and physical truth are different properties.
...
### Evidence versus communication dependency

```mermaid
flowchart LR
    L[Local autonomy] --> P[Can progress during silence]
    H[Historical evidence] --> C[Can check consistency later]
    HB[Continuous heartbeat / observation] --> K[More knowledge during silence]
    K -->|requires reachability| D[Communication dependency]
    P -. preserved by AIWA .-> Z[Partition tolerance]
```

More observability can reduce uncertainty, but mechanisms that require continuous reachability must not be confused with the autonomous core.
...
| Label | Meaning |
|---|---|
| **Proved** | Follows from the stated construction or mathematics. |
| **Tested** | Executed against the stated implementation or fixture. |
| **Constructed** | Implemented and exercised, but not a mathematical proof. |
| **Conditional** | Holds only under explicit assumptions or parameter conditions. |
| **Dampened** | An attack or weakness is reduced, but not eliminated. |
| **Open** | Not established by the present architecture or evidence. |
| **Not provided** | Outside what AIWA claims to establish. |
...
claims in this paper should be cited. The matrix states the status of each claim directly and distinguishes evidence from assumptions and open work.

| Claim | Status in the current specification | Evidence / limitation |
|---|---|---|
...
| Wall-clock manipulation cannot directly change reward | Architectural requirement, now backed by a real mechanism | §10, §16.2; cadence integrity (R11) is closed the current specification via a real sequential-hash-chain proof — see the honest, stated limits in §16.2 (not a true asymmetric VDF; hardware-relative difficulty; does not address parallel Sybil advancement across many domains) |
...
| checkSubmissionEligibility gates new module registrations | Wired into the mandatory pipeline as an optional, injected check; omitted entirely reproduces the open-by-default behavior | §27.4.1, Appendix H.20 |
...
| The compiled WASM binary loads correctly in a real browser | Confirmed directly in a real deployed session, confirmed to work from passing native tests | Appendix H.17; a live side-by-side comparison of WASM-backed vs. JS-backed computed results has not been run |
...
the principal open items: identity-churn's practical
exploitability (a new attack shape found while proving capital-splitting resistance,
not a known gap being restated), and canonical encoding/ordering as a
...
single most significant open item. See the notes at §10.1, §24.1, §24.6,
and §29.9 for what
...
### 18.1 Adversarial Test Matrix

The threat model becomes operational when each threat is tied to an invariant, evidence source, and remaining boundary. A test result is not promoted into a universal guarantee beyond its stated fault model.

| Threat | Required property | Current evidence | Remaining boundary |
|---|---|---|---|
| Concurrent double spend | `count(Consume(p)) ≤ 1` | Atomic consumption + non-atomic counterexample | Broader crash/failure models |
| Forged sender domain | Signature + full public-key/domain binding | Real Ed25519 adversarial test | Standard cryptographic assumptions |
| Duplicate delivery | Idempotent event identity | 500/500 convergence experiment | Permanent message loss / Byzantine delivery |
| Concurrent branch ordering | Same canonical materialization | V3 counterexample + JS/Rust parity | Normative order still open |
| Merge replay | One merge id consumed once | Protocol specified; V4 economic surrogate | Direct `ID_M` / `MergeSet` test open |
| Cadence acceleration | Monotonic bounded cadence + valid proof | Sequential hash-chain tests | Machine-checked rule not established |
| VDF reuse / wrong seed | Proof bound to domain and prior epoch | JS/Rust parity + rejection tests | Not an asymmetric VDF |
| Parallel Sybil cadence | Cost cannot be bypassed by one identity | One-domain sequential bound | Multiple identities can advance in parallel |
| Identity churn | New identities become economically harder over deployment age | Real-slot-indexed dampening | Legitimate late joiners pay same curve; mechanism off by default |
| Fake external burn | Verified external-chain evidence | Pure verifier + real transaction signing | Live RPC path not exercised in development environment |
| Stale RPC / network outage | Local protocol remains safe without RPC | Identity event separated from fold | Remote re-verification is optional |
| Malicious module | Capability boundary cannot be escalated by normal module API | Sandbox mechanism + hash verification | Browser-level escape/fuzz campaign open |
| Forged `postMessage` | Message source/instance boundary | Interface specified | Real-browser adversarial campaign open |
| Module code swap | Registered hash equals fetched bytes | Content-addressed loader/submission | Full browser/network attack surface |
| Smuggled contract condition | Minted condition is immutable | Generic-contract adversarial test | Stateful accumulation still needs dedicated reducers |
| Numeric divergence | Same protocol inputs produce same result | JS/Rust tested values | Normative numeric semantics open |
| Huge DAG / resource exhaustion | No trivial state corruption or replay amplification | Not exhaustively tested | Resource-bound / DoS analysis open |
...
### 22.1 Confirming the lemma (actual run).

With id_weak(e) = H(domain, payload) and r(b) = b , Lemma 1's condition holds by construction: any two events id_weak merges have equal payload and therefore equal reward.
...
```mermaid
flowchart TB
    A[Attacker] --> N[Choose number of identities N]
    A --> B[Choose capital b]
    N --> CID[Identity acquisition cost]
    B --> CAP[Capital acquisition cost]
    N --> R[Aggregate reward]
    B --> R
    T[Partition duration τ] --> R
    CID --> PROF[Net profit]
    CAP --> PROF
    R --> PROF
```

The relevant security question is not simply whether reward is concave. It is whether the complete attack remains profitable after identity and capital costs over the partition duration.

### Two distinct attack shapes

```mermaid
flowchart TB
    ATT[Attacker capital B] --> SPLIT[Split across N identities]
    SPLIT --> SAME[Same-age identities
capital splitting]
    SPLIT --> CHURN[Fresh identities
identity churn]
    SAME --> COST[Identity cost c_id]
    CHURN --> COST
    COST --> PROFIT[Attacker profit]
```

The current Proof-of-Will formula makes simultaneous fixed-capital splitting exactly reward-neutral before identity cost. The remaining economic concern is **identity churn**, where fresh identities avoid domain-age decay.
...
### 24.1 Adding identity cost to the fixed-capital case.

[Note on this section in the reference implementation: this specific derivation, and §24.2–24.3's that build on it, are stated for the power-law reference form r(b,q)=K·b^α·q^β is a power-law configuration alongside the Proof-of-Will configuration, where α governs b's exponent directly, and remain valid as the derivation for a deployment that chooses that simpler power-law form, which §10 supports as a valid deployment configuration. §10's current reference formula applies α to q instead, with b entering linearly; that structure is analyzed separately — see Appendix H.22 for the full argument. Summary of that result: under the idealizing assumption this section's own N* analysis already makes ("fixed t," i.e. identical elapsed age across all N split identities), the current reference formula's linear b-term makes splitting a fixed capital B into N simultaneous, equally-aged identities produce EXACTLY the same total reward regardless of N — verified algebraically and confirmed numerically to floating-point precision — which makes any c_id > 0 strictly, unconditionally splitting-discouraging, with optimal N*=1 for every parameter choice, a stronger and simpler result than this section's own conditional (0<α<1) case below. The analysis also identifies a genuinely new complication this section's original analysis never needed to consider, since the power-law form had no age-dependent decay term at all: because the current formula's reward decays with a domain's own age (§10), an attacker can sometimes profit from abandoning an aging domain and re-splitting the same capital across fresh, young identities instead — a decay-avoidance incentive distinct from simultaneous capital-splitting, confirmed numerically in Appendix H.22 to actually exceed identity cost for a realistic parameter choice. §24.6 now closes this, not by touching the reward formula's own domain-local age term at all, but by making the identity-cost burn required for a NEW registration a function of real, deployment-wide elapsed time — see that section and Appendix H.22's own updated derivation for the mechanism and its honest, stated limits.] Let an attacker hold total capital B , split evenly across N identities ( b = B/N each), and pay a fixed per-identity creation cost c_id > 0 . Profit as a function of N , for fixed B and elapsed age t :
...
### 24.2 The α trade-off, stated explicitly.
...
§24.1 assumed a fixed B merely redistributed across identities. A real attacker can instead acquire additional capital for each new identity. Let ψ(B_total) be a capital-acquisition cost. For N identities each acquiring capital b ( B_total = N·b ):
...
(or, if reward accrues continuously rather than only at partition end, E[∫₀^τ K·b^α·s^β ds] in place of K·b^α·E[τ^β] , depending on exact accrual mechanics — this paper does not commit to one). Here, the random variable is more precisely interpreted as Q, the
...
### 24.6 Identity-cost mechanisms an AIWA deployment can actually configure.

Five mechanism classes admit the closest
...
### Proof of Cost, Proof of Control, and Real-World Identity

The external-chain burn establishes an irrecoverable economic cost associated with a cryptographic key and, when verified, evidence that the key controlled the funds used for the transaction. It does **not** establish that the key corresponds to a unique human, organization, spacecraft, settlement, or other physical entity. AIWA therefore treats `c_id` as a **proof-of-cost / proof-of-control mechanism**, not proof-of-human identity.

This distinction is fundamental to the threat model: Sybil resistance is expressed as an economic property, while real-world uniqueness remains outside the protocol unless a deployment adds an external identity or credential system.
...
```mermaid
flowchart LR
    A[Domain A] --> Q[Durable outbound queue]
    Q -->|link available| N[Network / mesh backend]
    N --> B[Domain B]
    Q -->|link unavailable| Q
    N --> W[Watchdog / connection state]
```

Messages are queued before transmission is attempted. Transport availability changes delivery timing, not the validity model of the ledger.
...
activity while at least one peer was known, tears
...
```mermaid
flowchart TB
    K[Generate / unlock keypair] --> S[Sign burn transaction]
    S --> RPC[Solana network]
    RPC --> V[Verify transaction]
    V --> E[identity-register event]
    E --> H[Replicated DAG]
    H --> I[Materialized identity state]
```

The burn proof is recorded as a replicated event; the identity reducer remains chain-agnostic through the normalized transaction shape.
...
```mermaid
flowchart TB
    CODE[Third-party module code] --> HASH[SHA-256 hash check]
    HASH --> SB[iframe sandbox\nallow-scripts\nno allow-same-origin]
    SB --> CTX[Restricted ctx API]
    CTX --> HOST[Host application]
    HOST --> DAG[Validated DAG reducers]
    CODE -. never receives .-> PRIV[Direct authority over\nledger / keys / network]
```

The module is an untrusted extension. The host decides what crosses the boundary.


### 27.1 The module contract.

A module is loaded code that receives a context object ctx at activation and nothing else. ctx exposes: ctx.storage (a namespaced key-value store, scoped per-module, inaccessible to other modules); ctx.sendToPeer / ctx.onPeerMessage (real-time peer-to-peer messaging, routed through the transport layer of §25 without the module ever touching it directly, not durable, not part of H_d); ctx.toast (user notifications); ctx.commit / ctx.claim (accrual, §8); ctx.transferClaim (a real, signed Conservation transfer, §7 — the module names which claim and where, the host signs with the domain's real keypair, which the sandbox never holds); ctx.postCausalEvent / ctx.queryCausalState (general-purpose write and read for any causal-contract event type beyond what this paper's own reducers already define — see §27.8). A module cannot: read or write another module's storage; intercept or
...
The current reference implementation signs and posts a requested module transfer without an explicit user-facing confirmation dialog naming the amount and destination first. This remains an open human-confirmation requirement; module integrity is not equivalent to human authorization. Recorded here as an honest, open gap rather than silently assumed to already be true; a module that has passed integrity verification (§27's own sandboxing) is not the same guarantee as a human having confirmed a specific transfer, and this paper does not conflate the two.
...
### 27.5 Presentation independence.

The interface layer (theming, layout) is fully decoupled from module logic: a module's functional contract ( activate , deactivate , its rendering entry point) does not change based on which theme or presentation layer is active, and a presentation layer cannot
...
### 27.9 Composable verification primitives: the other half of §27.8's own scaling problem.
...
The Rust mirror (generic_contract_reducer.rs) mirrors the JS implementation — 8 tests mirroring the JS suite's own cases, including the identical end-to-end threshold-escrow demonstration, all passing on the first run. One real, stated divergence between the two mirrors, not hidden: the Rust side requires a condition template to already parse into the real Condition enum via an explicit, hand-written deserializer (parse_condition) — a genuinely stricter requirement than JS's permissive object literal, since Rust's own Condition type has no direct serde derive covering its exact field-per-variant shape without a larger refactor of causal_condition_evaluator.rs the current specification does not attempt. Executable detail for both languages, including the real end-to-end verification run against the actual application's own materialization logic (not a standalone unit-test shortcut), is in Appendix H.34.

### 27.x Module Security Boundary and Adversarial Coverage

The sandbox is an execution boundary, not a declaration of good behavior. The explicit adversarial surface includes `postMessage` source/channel confusion, replayed messages, cross-instance routing, oversized payloads, host-side object mutation, prototype-pollution paths, iframe navigation, network-based exfiltration, resource exhaustion, modules that never finish initialization, unbounded storage, and peer-message spam.

These are test targets, not claims that the current implementation has already eliminated every attack. The current evidence establishes content-addressed code integrity, the concrete sandbox mechanism, and the capability-limited `ctx` interface. Exhaustive browser-level adversarial testing remains open.
...
```mermaid
flowchart TB
    DATA[Local + replicated context] --> CTX[Context builder]
    CTX --> AI[Advisory AI]
    AI --> TXT[Suggestions / explanations]
    AI -. no write path .-> DAG[Consensus / ledger]
    AI -. no authority .-> SEC[Security decisions]
```

Removing the AI must not change what the protocol accepts as valid. That is a structural property, not merely a policy statement.


AIWA's actual AI layer has a single function: a brainstorming assistant suggesting new module ideas, grounded in a domain's own real registered modules and real contacts (§9), running entirely on-device via WebGPU (idea-agent.js, webllm-engine.js). Code generation and risk triage are not built. The structural rule below is unchanged and, if anything, easier to state precisely now that the scope is narrower: no function in §8–§24 — merge validity, conservation, consumption-uniqueness, or the economic-configuration validation of §27.2 — takes AI-derived input of any kind, and the idea agent has no path into any of them; it produces text a human reads, nothing else. This is what G5 (§3) means concretely: removing the AI layer entirely changes the user experience and removes a convenience, and changes nothing about what the platform will or will not accept as valid.
...
### 29.7 Hardware-backed key custody.

WebAuthn / secure- enclave integration as an alternative to mnemonic-plus- passphrase custody in the browser (§26).

### 29.8 Cadence-integrity mechanism.

The current specification: closed for the
...
the current specification; what R11 specifically added is that an active, honest-
...
dependency of economic accrual in the deployed architecture either.

### 29.9 Canonical encoding as a specified part of the wire protocol.
...
AIWA should not be summarized simply as “secure.” Its current status is more precise:

- **Strongest established layer:** event-set convergence and deterministic materialization under the tested delivery model.
- **Strong mathematical boundary:** unrestricted positive-rate autonomous issuance cannot coexist with a fixed global issuance bound under arbitrarily long partition without an additional scarcity constraint.
- **Implemented and adversarially tested:** conservation single-use behavior, sender-domain binding, formula immutability, module submission integrity, generic contract condition immutability, and cadence proof validation within their stated scopes.
- **Conditional:** Sybil and patient-capital security, because real-world identity and capital costs are deployment-dependent and random/adaptive partition duration remains open.
- **Dampened:** identity churn through deployment-wide, real-slot-indexed registration cost; this does not distinguish churn from legitimate late entry and is off by default.
- **Open protocol specification:** canonical byte encoding, canonical materialization ordering as normative requirements, numeric semantics, protocol versioning, and elevation of test vectors into a formal conformance suite.
- **Open adversarial boundary:** exhaustive browser-level module testing, resource-exhaustion analysis, direct merge-replay testing, and machine-checked verification.
- **Not provided:** proof of physical truth, proof of humanity, globally synchronized physical time, arbitrary autonomous issuance with fixed global supply, or hardware-grade browser key custody.
...
D.1 Partition issuance, worked by hand. With ρ_E = ρ_M = 1 unit/hour: after T=1000h, I_global=2000; after T=10000h, I_global=20000. No finite supply bound survives indefinitely under unrestricted autonomous issuance. With B_E = B_M = 5000, I_global ≤ 10000; once one domain exhausts its allocation, autonomous issuance from that domain stops, illustrating §13.1's trade-off. The executed simulation in Appendix H.4 reproduces these values as one of four tested policies.

D.2 Numerical illustration of N* (continuous relaxation, fixed t, α ≠ 1). This example uses the power-law reward configuration described in §24.1. K=1, B=1000, t=1, α=0.5, c_id=5 :
...
R11 Cadence integrity Sequential-hash-chain Closed, the current specification (§16.2). Bounds the rate of advancement, not a true asymmetric VDF (verification costs what computation costs); does not address parallel Sybil advancement across many domains (§24).
...
## Appendix H — Implementation Evidence

### Evidence pipeline at a glance

```mermaid
flowchart TB
    SPEC[Protocol specification] --> TEST[Executable tests]
    TEST --> PARITY[Cross-language parity]
    PARITY --> BROWSER[Browser / deployment checks]
    BROWSER --> LIMITS[Explicit limits and open properties]
    LIMITS --> CLAIMS[What the implementation actually establishes]
```

The appendix is intentionally read from the top down: first the evidence map, then the individual records only when a claim needs verification.


Appendix H is an evidence catalogue, not a development diary. Each subsection records a concrete implementation property, the checks performed, and the limits of what those checks establish. The order follows the dependency structure of the implementation rather than development chronology.

### Evidence map

| Area | Evidence sections | What to look for |
|---|---|---|
| Ledger and determinism | H.1–H.8 | event identity, merge, ordering, materialization |
| Conservation and identity | H.9–H.13, H.18–H.20 | proof-carrying transfer, identity cost, replicated reducers |
| Modules and contracts | H.14–H.17, H.21–H.24, H.31–H.34 | signing, hashing, sandboxing, contracts, verification |
| Transport and persistence | H.25–H.27 | durable queues, watchdog, persistence, presentation |
| AI and discovery | H.28–H.30, H.35–H.37 | advisory AI, profiles, trends, mechanism mining |
| Cadence and cross-language parity | H.30, H.33 | VDF, JS/Rust parity, regression coverage |

### Evidence notation

Use the evidence below as follows:

- **Implemented** — the mechanism exists in the reference implementation.
- **Tested** — an executable test exercises the stated property.
- **Parity-tested** — independent JS and Rust implementations agree on the tested vectors or operation.
- **Browser-tested** — the mechanism has been exercised in the deployed browser environment.
- **Open** — the implementation does not establish the stronger property claimed by the open item.

<details>
<summary>H.1 — Convergence Primitive</summary>
...
<details>
<summary>H.2 — Broken-Invariant Counterexamples (V0/V1/V3/V4)</summary>
...
<details>
<summary>H.3 — Weak Identity Under Time-Dependent Reward</summary>
...
<details>
<summary>H.4 — Issuance-Bound Simulation Under Four Scarcity Policies</summary>
...
Second, the same cadence defines protocol-recognized economic time.
...
leaving it implicit; the mechanism below closes that specific rate-of-advancement gap.
...
a separate identity-cost option is described in §24.6(ii):
a burn is never truly network-blocked, only slow to confirm, so "wait for
reconnection" is not an alternative for cadence. The
...
proof genuinely depends on the preceding epoch's real output rather than
...
pair — is in Appendix H.30. A genuine, dormant bug in
...
<details>
<summary>H.5 — Cross-Implementation Id Parity</summary>
...
<details>
<summary>H.6 — Cross-Language Confirmation of the Reward Function and Scarcity Policies</summary>
...
<details>
<summary>H.7 — Canonical Materialization Order Under Concurrent Branches</summary>
...
<details>
<summary>H.8 — Wall-Clock Reward: Deliberate Counterexample</summary>
...
structurally the the wall-clock formulation formulation this paper moved away from. This code
...
2. Counterexample. The broken wall-clock variant materializes the
   ...
   requirement to rule out, and precisely the vulnerability the wall-clock failure mode eliminated by deriving q from cadence rather than wall-clock
   time.
...
the wall-clock failure mode described in §10 reproduces exactly when
...
<details>
<summary>H.9 — Conservation: Consumption-Guard Counterexample and Cross-Language Parity</summary>
...
<details>
<summary>H.10 — External-Chain Burn as c_id: What Is and Isn't Verified</summary>
...
<details>
<summary>H.11 — Module Registry, Content Integrity, and the Sandbox Mechanism</summary>

security property registry ("unconfirmed").
...
What this closes, precisely: R19 reads "unconfirmed" because
...
One assumption recorded, now closed rather than left open:
...
<details>
<summary>H.12 — Module Registry Propagation, List Ranking, and Submission Eligibility</summary>

and its Rust mirror fold module-register/module-update/module-audit DAG
events into ModuleRegistryState, replacing the standalone
...
own formula produces, confirmed consistent by construction.
...
The following implementation properties are closed and tested. checkSubmissionEligibility() is wired into the real submission pipeline (submitModule() / submit_module(), via an injected function — see §27.4.1 and Appendix H.20 for the wiring itself and a real bug found while testing it end to end, with end-to-end tests), and the burn-registration flow is a real, reachable UI action (Commit → Burn), not confined to test fixtures. The application Desktop no longer auto-sorts pinned modules by rank. §27.7 explains the separation between informative rank and user-controlled layout.

</details>

<details>
<summary>H.14 — Proof-of-Will Reward Formula: Adoption, Parity, and What Remains Open</summary>

This section uses the Proof-of-Will formulation. reward.js and reward.rs were
rewritten from the power-law form r(b,q)=K·b^α·q^β to
...
the power-law form, where q=0 yields r=0 automatically whenever
...
What the current specification explicitly does NOT close, stated as plainly as the
...
against the power-law formula's b^α term; the current formula's b^1 term
...
<details>
<summary>H.15 — Domain Identity Is Derived, Not Named</summary>
...
<details>
<summary>H.16 — Conservation Wired to Economics: Sending Value Between Domains</summary>
...
recorded as the accurate current scope rather than implied
complete.

</details>

<details>
<summary>H.17 — The Real WASM Binary, Loading in a Real Browser</summary>

The compiled `.wasm` binary loads successfully in a real browser. The browser test validates the actual binary rather than only the adapter wiring; a live side-by-side comparison of WASM-backed and JS-backed materialized results remains open.
...
WASM output directory from an implementation when it genuinely was a
...
<details>
<summary>H.18 — Transfer Forgery Found and Closed: Ownership Requires Proof, Not a Claim</summary>
...
without proof of control, `from` would be whatever string the event author wrote, checked
against nothing. Found by testing the authorization path directly:
...
<details>
<summary>H.19 — Identity Cost Is Now a Materialized View, Not a Local Variable</summary>

Identity-cost registration is part of H_d and therefore propagates through reconciliation.
IdentityCostState lived in a standalone `let` in the
...
broadcastAndVerifyBurn, itself renamed and simplified in the current specification
...
imports were actually used, confirmed), not by a dedicated
...
<details>
<summary>H.20 — checkSubmissionEligibility Wired In, and a Real Bug Found Testing It</summary>

checkSubmissionEligibility() (module-rank.js / module_rank.rs) was
built and tested since an implementation but never actually called from
...
the closure entirely reproduces the fully open behavior exactly
...
A real bug was identified in the course of this, by testing the actual
...
<details>
<summary>H.21 — The Formula Made Immutable, and a Real Near-Miss Bug Found Verifying It</summary>
...
A real, functional near-miss was identified by testing the full switch
...
<details>
<summary>H.22 — Sybil-Profit Re-Derived for the Current Reference Formula, and a New Complication Found</summary>
...
entirely. Confirmed numerically, confirmed: with B=1000,
...
discard and re-mint rather than split-and-hold. The identity-churn question remains open: no claim is made about its practical magnitude under realistic partition timing and identity-cost calibration. The reproducible numeric case demonstrates that the attack can exceed identity cost under the stated parameters. Both results are backed by a permanent, executable regression
...
not a known one being restated.

The current specification: identity churn now has a real, tested,
...
being discarded in the deployed architecture — see solana-rpc.js's own header).
...
<details>
<summary>H.23 — The Issuing-Module Pathway Made Reachable, and Its Identity Scheme Displayed</summary>
...
built and tested since an implementation) had no path from the real
...
<details>
<summary>H.24 — Transport: The Interface, Delay-Tolerant Backend, and Watchdog</summary>
...
A real bug was found while writing these tests, by end-to-end testing: the
...
9 + 7), 194 JS tests in the project overall in the reference implementation.
...
<details>
<summary>H.25 — H_d Made Durable: Closing the "Only Survives While a Tab Is Open" Gap</summary>
...
The answer, before the reference architecture, was yes — and specifically,
...
main.js's restoreTipsFromDag() (a new method, not present)
...
directly: a domain restored from a stored history correctly
...
tests in the project overall in the reference implementation.

</details>

<details>
<summary>H.26 — Presentation Independence</summary>
...
module-sandbox.js's buildSandboxHtml() injects the active theme both ways this section's own
...
module-private), the one part of module-sandbox.js's
...
the project overall in the reference implementation — the last item on this
...
<details>
<summary>H.27 — The Idea Agent and Hyperprofile</summary>

implemented together after exploring of two much
...
tests total, 239 JS tests in the project overall in the reference implementation.

</details>

<details>
<summary>H.28 — Desktop Drag-and-Drop and Folders, and the Rank-Sort Conflict It Surfaced</summary>
...
A real, non-hypothetical conflict was identified in the course of this,
...
which — once a real operator-controlled arrangement mechanism existed to
...
new tests, 260 JS tests in the project overall in the reference implementation.

</details>

<details>
<summary>H.29 — The First Real Causal Contract, and the Generalization That Made It Scale</summary>
...
ticket count across many simulated trials, confirmed to.
...
A real, undiscovered bug was found and fixed during this
...
every caller in this codebase already does — with the new
...
the production reducer it verifies. 34 tests cover the contract-verification changes; the project test suite reports the current totals stated in the implementation status.

</details>

<details>
<summary>H.30 — Cadence Integrity Closed: A Sequential Hash Chain</summary>

Cross-Language Parity, and a Dormant Bug It Surfaced

The distinction between heartbeat observability and cadence-rate integrity is explicit: the heartbeat detects silence, while the sequential hash chain bounds the rate of economic epoch advancement.
...
on the preceding epoch's real output, not merely its existence — a
...
confirmed structurally: cadence_vdf.rs's own test suite
...
exactly this way in real CI the first time the current specification shipped. The
...
own @noble/hashes-backed output for the identical inputs, so no
...
test-vectors/g-scenario.json, identified by running the full suite
...
A genuine, dormant bug in cadence.rs was found in the
course of this, unrelated to the VDF mechanism's own logic: the
existing `self.domains.entry(domain).or_default()` line — present
before the reference architecture, never touched by the VDF work itself — reads the
...
function returns early. No test in the deployed architecture ever checked
...
harmless for every test's own assertions, which never inspected
...
Final counts in the reference implementation: 319 JS tests (12 new
...
tests (including cadence_vdf.rs and R11-specific
...
<details>
<summary>H.31 — Composable Verification Primitives, Validated Against a Real Contract's Own Logic</summary>
...
— is represented by a single protocol state; see Appendix H.32 for the
...
tests total across both languages the current specification: 365 JS tests, 185 Rust
...
<details>
<summary>H.32 — The Pool Reducer's Rust Mirror, and Two Real Gaps Its Construction Surfaced in conservation_bridge.rs</summary>
...
purpose; it was an omission, identified because pool_reducer.rs's own
...
reproduces the exact open-by-default behavior. 7 new tests cover both mechanisms:
...
Final counts in the reference implementation: 218 Rust tests total (
...
unchanged at 365 tests, since the work was entirely on the
Rust side. Zero warnings in either language's build.

</details>

<details>
<summary>H.33 — A Systematic JS/Rust Sweep, Not Just a Reaction to the Next Task</summary>
...
Final counts in the reference implementation: 230 Rust tests total (
...
<details>
<summary>H.34 — Permissionless Contract Minting: the Actual Last Mile</summary>
...
Final counts in the reference implementation: 238 Rust tests total (232 lib + 6 integration). JS: 378 tests in the generic-contract reducer suite and 415 JS tests in the project overall. Zero warnings in either language's build.

</details>

<details>
<summary>H.35 — The Idea Agent's Context, Deepened: Usage Over Registration, Recency Over All-Time, Multiple Contacts Over One</summary>

The idea-agent context is built from usage, recency, and multi-contact overlap signals.
...
property already true in the deployed architecture and unchanged by it), not
merely by continued assertion.

Final counts in the reference implementation: 19 tests in idea-agent.test.mjs (up
...
<details>
<summary>H.36 — A Real GitHub Bot, a Legal Correction Made Before Building, and a Deployment Bug Caught Before It Shipped</summary>

The external trend signal uses GitHub’s official public search API. LinkedIn scraping is not part of the architecture.
...
Final counts in the reference implementation: 25 tests in idea-agent.test.mjs (up
...
<details>
<summary>H.37 — Mining AIWA's Own Real Modules for Mechanism Patterns</summary>

the real source behind the YourMine reference (mine-patterns.js,
...
once the reference architecture, that code generation stays outside AIWA's actual,
...
Final counts in the reference implementation: 11 new tests in
module-pattern-miner.test.mjs, 5 new tests in idea-agent.test.mjs (30
total, in the current test suite). 415 JS tests in the project overall. Zero warnings.
