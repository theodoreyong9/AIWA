...
(Node, real @noble/hashes SHA-256) for an identical (seed, iterations)
pair, confirmed to match the Rust implementation's own output (real
sha2 crate) exactly, byte for byte, on the first attempt. A genuine
transcription slip was caught and corrected during this exact step —
an early draft of this test used a 63-character value, one hex
character short of a real SHA-256 digest, copied by hand rather than
verified programmatically; both Node's own `.length` property and an
independent Python length check confirmed the real value is 64
characters before the corrected value was used, rather than trusting
either the initial hand-transcription or a single tool's report of it.
