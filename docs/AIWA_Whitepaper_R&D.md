...
**A Reference Architecture for Autonomous, Delay-Tolerant Web Applications Under Arbitrarily Long Communication Partition**
...
AIWA is a research and development architecture, not a claim that interplanetary coordination, consensus, identity, economic security, or browser sandboxing have been solved in the general case.

Its purpose is more precise:

> **To make a browser-native distributed application architecture explicit enough that its invariants, assumptions, failure modes, economic constraints, and evidence can be independently examined and attacked.**

The architecture is built around one central separation:

```text
                    H_d
             replicated event history
                      │
                      ▼
...
- **replication** — whether domains can converge on the same event set under the stated delivery model;
- **deterministic materialization** — whether that event set produces the same derived state;
- **consensus** — whether that deterministic result is the semantically correct resolution of conflicting actions under the stated adversarial model.

The first two are implemented and experimentally exercised. The third remains a broader research question and is not inferred merely from determinism.

### Research programme

The principal questions are:

1. Under what assumptions can a merged event DAG provide useful semantic convergence?
2. Which concurrent economic conflicts require stronger coordination than canonical ordering?
3. What security properties does a sequential cadence proof provide when hardware and identity count are unconstrained?
4. What does an economically costly external-chain identity actually establish?
5. Can declarative contracts provide permissionless composition without executable code entering the trust boundary?
6. What complete browser security model is required around untrusted modules and `postMessage`?
7. Which protocol choices must become normative before independent implementations can safely interoperate?

AIWA's implementation, experiments, and open problems are organized around these questions.
...
### 8.2 H_d must survive the process that holds it, not only the domains it survives partition from. The requirement is this section's own framing had not made explicit: does H_d — the replicated event set this paper spends this much rigor establishing convergence properties for — actually persist anywhere durable, or does it exist only for the lifetime of whichever process currently holds it in memory? The event DAG must be durable; otherwise H_d lived in a plain in-memory structure, and unlocking a existing wallet reconstructed an empty ledger from scratch, same domain id, zero history, indistinguishable from a domain that had never done anything at all. Only the wallet key itself (browser localStorage) survived a closed tab; every cadence advance, every accrual, every burn's resulting identity registration, every minted formula, every registered module, every transfer sent or received, existed nowhere but a JavaScript variable. This is not merely inconvenient — for a paper whose central thesis is partition autonomy under arbitrarily long communication loss, failing to survive the mundane case of a closed browser tab on the SAME machine is a sharper version of the same failure mode §2's own G1 requirement exists to rule out, just applied to time instead of distance.
...
time-sensitive formulation. Under the wall-clock formulation, a manipulable local
...
constructs the wall-clock formulation-style wall-clock-dependent variant directly, shows it
...
### 10.1 The formula itself must be a durable protocol object, not a mutable local setting. The protocol requirement is that reward parameters cannot be changed by mutable local state. They must be bound to an immutable protocol object. The reward parameters must not be mutable local state — θ's reward parameters (α, β, γ, C, minQ) lived as a plain, freely-editable local variable, never a DAG event. Two domains could silently disagree about what the exact same accrual event was worth, simply because one had a different value typed into a settings field than the other — a real fork, just never named as one, since nothing about H_d's own convergence guarantee (§9) protects a value that never entered H_d to begin with.
...
AIWA uses a deliberately non-uniform vocabulary for claims:

| Status | Meaning |
|---|---|
| **Proved** | Follows from the stated model or invariant |
| **Constructed** | Implemented as an explicit mechanism |
| **Tested** | Observed in executable tests for the stated fault class |
| **Conditional** | Holds only under explicit deployment or economic assumptions |
| **Dampened** | An observed attack is materially reduced, but not eliminated |
| **Open** | Not yet demonstrated or not yet specified |
| **Not provided** | Outside the guarantees of the architecture |

This distinction is essential. In particular:

- **deterministic materialization is not general consensus**;
- **a hash-chain cadence proof is not an asymmetric VDF**;
- **an external-chain burn is proof of cost/control, not proof of human identity**;
- **a cryptographic commitment provides integrity relative to its anchor, not physical truth**;
- **an iframe sandbox is not, by itself, a complete browser threat model**;
- **test counts are evidence about tested cases, not a proof of the absence of vulnerabilities**.

The adversarial surface includes duplicate/replayed events, concurrent spends, forged domain labels, malicious reconciliation, cadence acceleration, identity churn, patient capital, external-chain evidence failures, malicious modules, forged or replayed `postMessage` traffic, resource exhaustion, numeric divergence, and protocol-version mismatch.

The appropriate research unit is:
...
A claim is considered mature only when its evidence is identified at the same level of precision as the claim itself.
...
### 29.8 Cadence-integrity mechanism. The current specification: closed for the
...
AIWA is intentionally structured for independent criticism.

The most valuable contributions are not limited to feature additions. They include:

- counterexamples to convergence or conservation claims;
- adversarial DAG scenarios;
- formalizations of the protocol invariants;
- independent JS/Rust/WASM or non-reference implementations;
- fuzzing and property-based testing;
- browser and sandbox security research;
- economic simulations of Sybil, churn, and patient-capital attacks;
- transport implementations for delay-tolerant environments;
- protocol conformance vectors;
- proofs that a mechanism is sufficient, or precise demonstrations that it is not.

A convincing counterexample is a first-class research result.

The project should therefore be evaluated less by the number of features it contains than by how precisely it exposes the boundary between **what is established, what is assumed, and what remains to be demonstrated**.
...
## H.8 — Wall-Clock Reward: Deliberate Counterexample
...
structurally the wall-clock formulation this paper moved away from. This code
...
the implementation note describing the previous desktop behavior
