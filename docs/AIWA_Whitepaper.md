...
constructs the the wall-clock formulation-style wall-clock-dependent variant directly, shows it
...
Appendix H is an evidence catalogue, not a development diary. Each subsection records a concrete implementation property, the checks performed, and the limits of what those checks establish. The order follows the dependency structure of the implementation rather than a chronology of revisions.
...
leaving it implicit; the mechanism below
direct question, followed by one direct pushback on an initially-incomplete
answer, led to it being closed in the current specification.
