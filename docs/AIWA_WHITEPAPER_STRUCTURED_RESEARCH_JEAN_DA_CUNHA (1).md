...
AIWA is intentionally open to independent implementation and modification.
...
A fork may change implementation details, economics, modules, transport or protocol rules and therefore become incompatible.

The research hypothesis is:

> **A sufficiently explicit public protocol boundary may allow independently operated implementations to interoperate without requiring a central operator to coordinate every participant.**

For that hypothesis, the relevant public boundary would include:

- event schemas;
- canonical encoding;
- event identity;
- signature and domain-binding rules;
- canonical materialization;
- conservation transitions;
- causal verification semantics;
- numeric semantics;
- version negotiation;
- rejection semantics;
- conformance vectors.
...
The existing JS/Rust parity work is useful evidence that shared protocol computations can be represented across implementation environments. It is not proof of interoperability with arbitrary future implementations.

The decisive future experiment is an independent implementation built from the protocol boundary and conformance vectors, followed by cross-implementation event-identity, replay, materialization and invariant tests.

This proposal does not introduce a new network type or consensus mechanism. It is a research extension of the existing event-history, deterministic-materialization and verification architecture.
