...
> A technical reference for AIWA's architecture, protocol mechanics, implementation boundaries, security model, testing and research direction.

## How to read this README

This README is organized from **why AIWA exists → how the protocol works → how value and identity move → how peers and modules interact → how applications are built → what can fail → how the repository is tested and deployed → what remains a research question**.

---

## Table of Contents

- **PART I — PURPOSE, CLAIMS AND CONCEPTUAL MODEL**
...
- **PART X — CONCLUSION AND RESEARCH DIRECTION**
  ...
  - [42. Forkability and public interoperability](#42-forkability-and-public-interoperability)

### Reading order

1. Start with Parts I–II for the model and core data architecture.
2. Read Parts III–IV for economics, cadence, identity and conservation.
3. Read Parts V–VII for transport, modules, contracts and application layers.
4. Read Part VIII for counterexamples, threats and security boundaries.
5. Finish with Parts IX–X for reproducibility, deployment, participation and the forkability/interoperability research hypothesis.
...
## 42. Forkability and public interoperability

AIWA is deliberately open to independent implementation and modification.
...
A fork may change its implementation, economics, modules, transport or other behavior and therefore become incompatible with the reference implementation.

The stronger research question is whether a sufficiently explicit public protocol boundary could allow independently operated implementations to remain interoperable.

For AIWA, the relevant boundary is made of protocol-visible rules such as:

- event identity;
- canonical encoding;
- canonical materialization order;
- signatures and domain authorization;
- conservation transitions;
- causal-contract semantics;
- numeric semantics;
- versioning;
- conformance vectors.
...
This is a **research hypothesis**, not a claim that the current repository already constitutes a universal interoperability standard.

The current repository already provides an important precursor: the JavaScript and Rust implementations are checked against shared vectors and parity scripts. A future independent-implementation conformance suite would be a stronger test of public interoperability.

The important property of forkability is therefore not that every fork remains identical. It is that the protocol can potentially remain useful even when implementation and operational authority are distributed among independent parties.

---

# DOCUMENT MAP

The README can be read as the following dependency chain:
...
Repository / core architecture
    ↓
Event DAG + canonical materialization
    ↓
Economics + cadence + scarcity
    ↓
Identity + conservation
    ↓
Transport + modules + contracts
    ↓
Application / AI / Rust-WASM
    ↓
Counterexamples + threat model + security boundaries
    ↓
Tests + build + deployment
    ↓
Participation
    ↓
Forkability / public interoperability (research hypothesis)
```

This map is a reading aid. It does not add a new architectural layer to AIWA.
